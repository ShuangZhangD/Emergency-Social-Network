...
class User {
    ...
    removeUser(db, callback) {
      this.collection = db.collection('USERS');
      collection.remove({"username" : this.username}, function(err, results) {
        if(err)
        {
            console.dir('Error creating a new user'+ err);
            callback(null, err);
    ...
    updateStatus(db, username, status, callback) {
      this.collection = db.collection('USERS');
      collection.update({"username" : username}, {$set : { "status" :  status}}, function(err, results) {
        if(err)
        {
    ...
    displayStatusUsers(db, status, callback) {
      this.collection = db.collection('USERS');
      collection.find({"status" : status}, function(err, results) {
        if(err)
        {
            console.dir('Error updating the status ' + err);
            callback(null, err);
